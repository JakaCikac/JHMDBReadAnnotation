...
% * 2.4 - 16/04/2016 Jaka Cikac
%         Exclude ".AppleDouble" dir.
% 
% *Examples*
%
%   D = rdir('*.m');
...
if isempty(wildpath)
  ...
  % Exclude ".", ".." and ".svn" and ".AppleDouble" directories from the list
  excl = isdotdir(D) | issvndir(D) | isAppleDoubledir(D);
  D(excl) = [];
...
elseif strcmp(wildpath,'**')
  ...
  % Exclude ".", "..", ".svn", ".AppleDouble" directories and files from the list
  excl = isdotdir(D_sd) | issvndir(D_sd) | ~([D_sd.isdir]') | isAppleDoubledir(D_sd);
  D_sd(excl) = [];
...
%---------------------------- end of subfunction --------------------------
%% ------------------------------------------------------------------------
function tf = isAppleDoubledir(d)
% True for ".AppleDouble" directories.
% d is a structure returned by "dir"
%

is_dir = [d.isdir]';

is_AppleDouble = strcmp({d.name}, '.AppleDouble')';
%is_AppleDouble = false; % uncomment to disable ".AppleDouble" filtering 

tf = (is_dir & is_AppleDouble);
